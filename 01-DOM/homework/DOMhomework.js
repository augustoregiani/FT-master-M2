...
created.innerHTML = 'Aplicacion creada por augusto'
...
//      esta función conpleteTodo como callback

//<div class='toDoShell'></div>
//<span>toDo.descripccion</span>
function buildToDo(toDo, index) {
  // Tu código acá:
  let toDoShell = document.createElement('DIV');
  //toDoShell.classList='toDoShell';---> es una opccion.
  ...
  toDoText.addEventListener('click' , completeToDo) //cuando clikee en el span, se va a ejecutar completeToDo, busca el indice del evento que marco el evento, y le marca como true, el complete. 
                                                    //este ultimo paso lo que va a hacer es tacharme la tarea, como si estuviese completada
                                                    // se tacha por que, si miraos en la hoja de stylos (/assets/style/css) , tiene la clase .completeText
                                                    //y en la misma le cambia el color, y le agrega un text-decoration: line- throungh (tachado)
...
//esto recibe un arreglo llamado ToDos(que adentro tienen un objeto toDo, por lo que tiene una descripccion y tiene complete)

evento.addEventListener( 'click', addToDo);
var array= toDos.map(function(toDo, index){  return  buildToDo(toDo, index)})
return array;
}
//toDos = [{descrioccion: ..., complete: ...}, {...} , {...} , {...}] array de objetos ToDos
//array = [<div><span></span></div>  , <div><span></span></div>  , <div>....</div> , .... ]
...
function addToDo() {
  ...
  //o bien puedo no poner .value, y declarar aca:
  //let valorInput = input.value;
  if (input.value != ''){
  let nuevoToDo = new ToDo (input); //aca input, ya tiene a el selector #todoInput.value, por lo que la descripccion para el nuevo nodo ya fue obtenida.
  toDoItems.push(nuevoToDo);//toDoItems es el arreglo vacion que teniamos arriba y al cual le seteamos el display para que lo muestre en pantalla.
  toDoInput.value='';//con esto, seteamos el string que se escribio se borre despues del evento.
  displayToDos();
}
...
function completeToDo(event) { //event, es el evento en si, y tiene pripiedades propias de los eventos, como target.
  const index = event.target.id; //esto nos va a guardar el id, de la tarea que se realizo, que se guarda en target.
  // Tu código acá:
  toDoItems[index].completeToDo(); //completeTodo me seteaba el complete, como true.
  displayToDos();}
