...
### Única fuente de verdad (Single source of truth) Es store es UNICO objeto. Y lo que diga el store es verdad. 
...
store.dispatch({
  type: 'COMPLETE_TODO',   
  index: 1     ---> Aca la informacion necesaria para que s emodifique el reducer
})
...
FUNCION ACCION CREATOR  ---> estas son las funciones que se llaman en el dispatch
function complete_Tarea (){
  return {
    type: 'complete_todo'
    payload: 1 
  }
...
una funcion pura es una func qe recibe los mismos inputs, devuelve los mismos outputs
